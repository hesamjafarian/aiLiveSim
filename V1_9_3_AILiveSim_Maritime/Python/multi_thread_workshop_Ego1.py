...
def control_boat(vehicle_id, control_socket, sync_event):
    last_status = SimulationContext.vehicleStatus[vehicle_id].SimulationTime
    ...
    while True:
        ...
        position, forward, right = None, None, None

        if last_status != SimulationContext.vehicleStatus[vehicle_id].SimulationTime:
            last_status = SimulationContext.vehicleStatus[vehicle_id].SimulationTime
            parsedJson = json.loads(SimulationContext.vehicleStatus[vehicle_id].StatusString)
            position = parsedJson['Position']
            forward = parsedJson['Forward']
            right = parsedJson['Right']
            SimulationContext.vehicleStatus[vehicle_id].CurrentPosition = position
        ...
        if 50.0 < dist_to_target < 2000.0:
            if SimulationContext.vehicleStatus[vehicle_id].num_waypoints_reached < len(
                    SimulationContext.vehicleStatus[vehicle_id].waypoint_list):
                SimulationContext.vehicleStatus[vehicle_id].num_waypoints_reached += 1
                if SimulationContext.vehicleStatus[vehicle_id].num_waypoints_reached < len(
                        SimulationContext.vehicleStatus[vehicle_id].waypoint_list):
                    SimulationContext.vehicleStatus[vehicle_id].CurrentDestination = \
                    SimulationContext.vehicleStatus[vehicle_id].waypoint_list[
                        SimulationContext.vehicleStatus[vehicle_id].num_waypoints_reached]
                    dist_to_target = SimulationContext.vehicleStatus[vehicle_id].get_distance_to_target()
                    #print(f"Updated distance to target for {vehicle_id} = {dist_to_target}")

        if position and forward and right and SimulationContext.vehicleStatus[vehicle_id].num_waypoints_reached < 4:
